...
import { Briefcase, Lock, Mail, ArrowRight, Sparkles, CheckCircle2, Shield } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 sm:p-10 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50">
        ...
        <div className="p-3.5 rounded-2xl bg-brand-50/70 border border-brand-100 space-y-2">
          <p className="text-[11px] font-bold text-brand-800 uppercase tracking-wider flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-brand-600" /> Fast 1-Click Demo Accounts:
          </p>
          <div className="grid grid-cols-3 gap-1.5 text-xs">
            <button
              type="button"
              onClick={() => handleQuickLogin('alex@example.com', 'password123')}
              className="py-1 px-2 rounded-lg bg-white text-slate-700 font-semibold border border-brand-200 hover:bg-brand-600 hover:text-white transition-colors"
            >
              Seeker
            </button>
            <button
              type="button"
              onClick={() => handleQuickLogin('sarah@techcorp.io', 'password123')}
              className="py-1 px-2 rounded-lg bg-white text-slate-700 font-semibold border border-brand-200 hover:bg-brand-600 hover:text-white transition-colors"
            >
              Recruiter
            </button>
            <button
              type="button"
              onClick={() => handleQuickLogin('admin@hireloop.com', 'admin123')}
              className="py-1 px-2 rounded-lg bg-white text-slate-700 font-semibold border border-brand-200 hover:bg-brand-600 hover:text-white transition-colors"
            >
              Admin
            </button>
